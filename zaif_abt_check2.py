...
#bet money
        ...
        bet = int(min(money,min_depth))
        bet_min = btc_jpy_pairs[0]['item_unit_min'] * btc_jpy_depth["bids"][0][0]
        if bet < bet_min:
            print('no {3} min_depth:{0} money:{1} bet:{2}'.format(min_depth,money,bet,coin))
            continue
        print('{3} min_depth:{0} money:{1} bet:{2}'.format(min_depth,money,bet,coin))
...
#        print( '{0}jpy > {1}alt > {2}btc > {3}jpy : result {4}%'.format(bet,ja,jab,jabj, Profits_jabj ))
        ...
        if Profits_jbaj > 100.5:
            count = count + 1
            print( '{0} {1} count:{2}'.format(coin, time.ctime(),count) )
